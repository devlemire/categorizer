...
During this project we'll be building a web application that allows for categorizing information in radar charts. We'll build out the three main components to make our web application to work. The component that allows for making new charts, the component that displays a list of the created charts, and the component that allows us to add datasets to created charts. To keep track of data and pass it to the correct components we'll make heavy use of Redux and React Redux.
...
* Create a `constructor` method:
  * This method should have one parameter called `props`.
  * This method should call `super` and pass `props` in as the first parameter.
  * This method should create a state object with two properties:
...
* Open `src/components/AddDataset/AddDataset.js`.
* Create a map just above the `button` with a `className` of `"add-dataset__submit"`:
...
Let's begin by opening `src/components/AddDataset/AddDataset.js`. Then map over `labels` just above the `button` with a `className` of `"add-dataset__submit"`. This map should keep track of the `label` and the `index`:
...
<summary> <code> AddDataset.js </code> </summary>
